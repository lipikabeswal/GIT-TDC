...
import com.ctb.tdc.web.utils.TuningUtilityServletUtils;;

  
/**
 * @author John_Wang
 */
public class TuningUtilityContentServlet extends HttpServlet {

	private static final long serialVersionUID = 1L;

	//static Logger logger = Logger.getLogger(TuningUtilityContentServlet.class);

	/**
	 * Constructor of the object.
	 */
	public TuningUtilityContentServlet() {
		super();
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String method = TuningUtilityServletUtils.getMethod(request);
		System.out.println("Go.... doGet........");
	 if (method.equals(TuningUtilityServletUtils.GET_ITEM_METHOD)) {
			getItem(request, response);
		} else if (method.equals(TuningUtilityServletUtils.GET_IMAGE_METHOD)) {
			getImage(request, response);
		} 
		else if (method.equals(TuningUtilityServletUtils.GET_LOCALRESOURCE_METHOD)) {
		     getLocalResource(request,response);
		} else {
			TuningUtilityServletUtils.writeResponse(response, TuningUtilityServletUtils.ERROR);
		}
	...
	private void getItem(HttpServletRequest request,
		...
		String itemxml = null;
		System.out.println("Go.... doGet........"); 
		String folder = TuningUtilityServletUtils.getFolder(request);
		if (folder == null || "".equals(folder.trim())) // invalid image
			throw new IOException("No folder in request.");
		
		try {	
				itemId = TuningUtilityServletUtils.getItemNo(request);
			...
			/*logger.error("Exception occured in getItem() : "
					+ TuningUtilityServletUtils.printStackTrace(e));
			TuningUtilityServletUtils.writeResponse(response, TuningUtilityServletUtils.ERROR);*/
			e.printStackTrace();
		}
	}
	...
	private void getImage(HttpServletRequest request,
		...
		String xml = TuningUtilityServletUtils.getXml(request);
		String image;
		...
		try {
			if (xml == null) {
				image = TuningUtilityServletUtils.getImageId(request);
			}
			else {
