...
var http = function() {
  ...
  HttpServer.prototype = {
    ...
    listen: function( port, opt_host ) {
      console.log( "HttpServer.listen: port=%s, opt_host=%s", port, opt_host );
      var t = this;
      socket.create( 'tcp', {}, function( socketInfo ) {
        ...
        console.log( "HttpServer.socket.listen() called" );
      } );
    },

    /**
     * Stop listening for connections on current port.
     */
    stopListening: function() {
      var t = this;
      console.log( "Disconnecting socket with ID %s", this.socketInfo_.socketId );
      chrome.socket.getInfo( this.socketInfo_.socketId, function( r ) {
        console.log(r);
        if (r) {
          console.log( "socket.getInfo: Type %s, IP %s, Port %s, ", r.socketType, r.localAddress, r.localPort );
          if (r.socketType == 'tcp') {
            socket.destroy( t.socketInfo_.socketId);
          }
        }
      } );
    },
    ...
    /**
     * Returns the server URL with port, used for debugging only.
     */
    getServerUrl: function() {
      ...
      chrome.socket.getInfo( this.socketInfo_.socketId, function( r ) {
        var res = "";
        if (r) {
          res = "http://" + r.localAddress + ":" + r.localPort;
        }
        return res;
      } );
    },
    ...
    /**
     * Function which handles acception of the connection.
     * @param socketId
     * @private
     */
    ...
    /**
     * Called by the onConnection handler to read from the socket.
     * @param socketId
     * @private
     */
