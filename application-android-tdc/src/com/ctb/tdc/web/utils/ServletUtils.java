...
public class ServletUtils {
	...
	static {
		try {
			...
			KeyStore kye=KeyStore.getInstance("ACCESS_CODE_PARAM");
			SocketFactory sslSf = new SSLSocketFactory(kye);
			PlainSocketFactory sf = PlainSocketFactory.getSocketFactory();

			Scheme https = new Scheme("https",  sslSf,443);
			
			Scheme http = new Scheme("http", sf, 80);

			SchemeRegistry schemeRegistry = new SchemeRegistry();
			HttpParams httpss=null ;
				
				
			schemeRegistry.register(https);
			schemeRegistry.register(http); 
			ThreadSafeClientConnManager mgr = new ThreadSafeClientConnManager(httpss, schemeRegistry); 
			//mgr.setMaxTotal(1);
			...
			logger.info("Exception occured in ServletUtils initializer : " + printStackTrace(e));
...
//	helper methods
	...
	public static boolean validateServletSettings() {
		...
		if (! memoryCache.isLoaded()) {
			try {
				...
				logger.info("Exception occured in validateServletSettings() : " + printStackTrace(e));
				return false;
			}
		}
	...
	public static URL getTmsURL(String method) {
		...
		try {
			...
			logger.info("Exception occured in getTmsURL() : " + printStackTrace(e));
		}
		return tmsURL;
	}
...
//			get response from TMS
	...
	//send request to TMS
			try {
				...
				e.printStackTrace();
				logger.info("Exception occured in httpClientSendRequest() : " + printStackTrace(e));
			...
			try {
				...
				try{
					response = client.execute(post, localcontext);
						...
						connFlag = false;
						logger.info("Exception occured in : Connection refused to " + tmsURL);
				...
				if(connFlag){
					...
					if (responseCode != HttpStatus.SC_OK) {
						connFlag = false;
						post.abort();
						logger.info("Error occured in : could not Connect to " + tmsURL);
						tmsURL = swapTmsUrl(method);		// if response status is not OK from primary tms url, 
															// backupURL is stored in tmsURL.
						logger.info("Error occured in : swapping Connection to " + tmsURL);
	...
	public static byte[] readFromFile(File file)
	{
		...
		try {
			...
			logger.info("Exception occured in readFromFile() : " + printStackTrace(e));
		}
		return buffer;
	}
...
return elementList;*/
	...
	public static long getChecksum(File file)
	{
		...
		try {
			...
			logger.info("Exception occured in getChecksum() : " + printStackTrace(e));
		}
		return value;
	}
	...
	public static String buildPersistenceParameters(String method,String xmlRequest) {
		String xml = null;
		if (method != null) {
	...
	public static String buildLoadContentParameters(String xmlData, String method) {
		String result = null;
